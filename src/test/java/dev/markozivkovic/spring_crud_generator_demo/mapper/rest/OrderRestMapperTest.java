...
class OrderRestMapperTest {
    ...
    @Test
    void mapOrderTOToOrderPayload() {

        final OrderTO orderTO = Instancio.create(OrderTO.class);

        final OrderPayload result = this.orderMapper.mapOrderTOToOrderPayload(orderTO);

        verifyOrderPayload(result, orderTO);
    }

    @Test
    void mapOrderTOToOrderPayload_list() {

        final List<OrderTO> orderTOs = Instancio.ofList(OrderTO.class)
                .size(10)
                .create();

        final List<OrderPayload> results = this.orderMapper.mapOrderTOToOrderPayload(orderTOs);
    ...
    private void verifyOrderPayload(final OrderPayload result, final OrderTO orderTO) {
